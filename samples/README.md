...
| 17 | [`17_tcp_cluster_mesh`](17_tcp_cluster_mesh/main.cpp) | a 4-node full mesh over real TCP: 16 send lanes fire at once (12 simultaneous first-dials across 6 pairs), stressing dial-dedup convergence, per-(sender,receiver) FIFO, and hang-freedom (one combined bounded poll, not one per node) | 010, 019, 021 |
| 18 | [`18_tcp_cluster_multiprocess`](18_tcp_cluster_multiprocess/main.cpp) + [`run_cluster.sh`](18_tcp_cluster_multiprocess/run_cluster.sh) | the same 4-node full mesh as 17, but as REAL SEPARATE OS PROCESSES (no shared address space) — each process derives its placement table independently, dials peers that may not be listening yet (021 reconnect covers it), and holds a shared settle window so no process's early exit yanks a socket out from under a slower peer | 010, 019, 021 |
| 19 | [`19_tcp_cluster_dynamic_join`](19_tcp_cluster_dynamic_join/main.cpp) + [`run_cluster_dynamic.sh`](19_tcp_cluster_dynamic_join/run_cluster_dynamic.sh) | a node joins a cluster that is ALREADY RUNNING, live, no restart of the existing processes — wires the real `SwimMembership` (021 gossip/join protocol) together with `net::TcpTransport` for the first time in these samples; the newcomer is fully sending/receiving within its own settle window, proven by the same base-node-registered Loggers every other TCP sample uses | 010, 019, 021 |
| 20 | [`20_swim_membership_stability`](20_swim_membership_stability/main.cpp) | an ISOLATED instance with no actors, no traffic, no multi-process teardown — one process samples roster/epoch/connections every 250ms for 10s across a live join, to answer cleanly: does real-time SWIM+TCP ever churn on its own? (No — converges once, holds flat.) | 019, 021 |
| 21 | [`21_teardown_skew_repro`](21_teardown_skew_repro/main.cpp) + [`run_repro.sh`](21_teardown_skew_repro/run_repro.sh) | isolates the OTHER half of 20's question: deliberately reproduces the anti-pattern samples 18/19 originally had (every process exits the instant it's personally satisfied, no shared settle window) to prove — not just suspect — the mechanism behind their flaky teardown-time readings: a JoinAck digest is a point-in-time snapshot, and a joiner processed before the seed learned of its siblings is permanently stranded if the seed leaves before its next gossip round | 019, 021 |
| 22 | [`22_tcp_secure_transport`](22_tcp_secure_transport/main.cpp) | a PROTECTED connection: `SecureTransport` (020) wrapping the real `net::TcpTransport` for the first time in these samples (existing security tests only ever wrap the loopback double) — confidentiality (sealed bytes never equal plaintext), integrity (a captured-and-tampered frame is rejected before reaching the actor), replay protection (a captured-and-resent frame is rejected), and key isolation (a rogue node with a real, fully-established TCP connection but the wrong shared key cannot forge an accepted frame) | 019, 020 |
