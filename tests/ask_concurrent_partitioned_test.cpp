// Same shape as ask_concurrent_test.cpp (ADR-007 C2: many callers ask ONE actor concurrently; each
// caller's block_on must resolve to ITS OWN correct reply) but with a PARTITIONED MessagePool
// (num_partitions == caller thread count), proving the real ask/reply win-arbitration path — not
// just raw acquire()/reclaim() — is unaffected by partitioning. Run under TSan/ASan. Capped at 4
// caller threads (machine-safety: never saturate cores).
#include <atomic>
#include <cstdio>
#include <memory>
#include <thread>
...
int main() {
    ...
    // The only difference from ask_concurrent_test.cpp: num_partitions == kThreads, so each caller
    // thread gets its own partition (message_pool.hpp's thread-lane keying) instead of sharing one.
    detail::MessagePool pool(8192, kThreads);
    Server actor;
    ...
    std::printf("ask_concurrent_partitioned_test: %s  (threads=%d each=%d misroutes=%d failures=%d)\n",
