...
- **D1 — JumpTable-Dispatch** (WINNER): each actor declares `using protocol =
  ...
  `.rodata` function-pointer indexed call `thunks[slot](https://github.com/thnak/QuarkCpp/tree/master/decisions/self,payload,ctx)`. Typed
...
- **Extended by [ADR-018](ADR-018-outbound-streaming-replies) (streaming replies).** The
...
- **Best-effort broadcast reuses this dense-slot dispatch.**
  [ADR-019](ADR-019-best-effort-broadcast-publish-primitive) (`Topic<M>`) stamps its
