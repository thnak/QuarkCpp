...
[006](006-Messaging-and-Addressing) records as "streaming replies" and leaves
unspecced for the **inbound** direction.

> **Design pinned by [ADR-005](ADR-005-inbound-stream-ingestion-hot-path)**
...
[007](007-Failure-and-Supervision) error plus a debug assertion. **Multi-source
...
- **Outbound streaming replies** (an `ask` returning a stream) — **now specced**, as the
  ...
  [ADR-018](ADR-018-outbound-streaming-replies)). This spec's *primitive* is
