...
snapshot/event-log/recovery code in [`snapshot.hpp`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/snapshot.hpp) and
[`event_log.hpp`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/event_log.hpp) never changes.
...
The `Store` concept ([`persistence.hpp`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/persistence.hpp)) is a **compile-time
...
shared conformance harness ([`tests/adapters/store_conformance.hpp`](https://github.com/thnak/QuarkCpp/blob/master/tests/adapters/store_conformance.hpp))
...
|---|---|---|---|---|
| [`InMemoryStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/persistence.hpp) | RAM (`unordered_map`) | No (process-lifetime) | std-only | **shipped** — the reference; exercises every seam property |
| [`FileStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/file_store.hpp) | append-only WAL file per actor + `fdatasync` | **Yes** | std-only + POSIX | **shipped + verified** (`persistence_filestore_durable_test`) |
| [`SqliteStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/adapters/sqlite_store.hpp) | SQLite (embedded, single-file, ACID) | **Yes** | `libsqlite3` (opt-in) | **shipped**, built when `QUARK_WITH_SQLITE=ON` |
| [`RocksStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/adapters/rocksdb_store.hpp) | RocksDB (embedded LSM KV) | **Yes** | `librocksdb` (opt-in) | **shipped**, built when `QUARK_WITH_ROCKSDB=ON` |
...
Durable **reminders** (027 / [ADR-017](ADR-017-durable-reminder-mass-due-scale-gate))
...
([`reminder_service.hpp`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/reminder_service.hpp)) is kept minimal and distinct — five
methods:
...
|---|---|---|---|---|---|
| [`InMemoryReminderStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/reminder_service.hpp) | RAM (`unordered_map`) | No (process-lifetime) | in-RAM `std::map` (service) | std-only | **shipped** — the reference |
| [`FileReminderStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/core/reminder_service.hpp) | CRC32-framed append-only WAL (`.qrem`) + `fdatasync` | **Yes** | rebuilt on open (`compact()` bounds it) | std-only + POSIX | **shipped + verified** (`reminder_store_conformance_test`) |
| [`SqliteReminderStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/adapters/sqlite_reminder_store.hpp) | SQLite (embedded, single-file, ACID) | **Yes** | native `rem_due` SQL index | `libsqlite3` (opt-in) | **shipped**, built when `QUARK_WITH_SQLITE=ON` |
| [`RocksReminderStore`](https://github.com/thnak/QuarkCpp/blob/master/include/quark/adapters/rocksdb_reminder_store.hpp) | RocksDB (embedded LSM KV) | **Yes** | native — big-endian due prefix in the key | `librocksdb` (opt-in) | **shipped**, built when `QUARK_WITH_ROCKSDB=ON` |
...
([`tests/adapters/reminder_store_conformance.hpp`](https://github.com/thnak/QuarkCpp/blob/master/tests/adapters/reminder_store_conformance.hpp)) —
...
4. Add a `QUARK_WITH_MYSTORE` option + `find_package` block in
   [`cmake/QuarkPersistenceAdapters.cmake`](https://github.com/thnak/QuarkCpp/blob/master/cmake/QuarkPersistenceAdapters.cmake) and a
