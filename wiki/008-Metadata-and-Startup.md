...
**Node/Shard resource resolution (ADR-021, proven).** Node- and Shard-scoped
resource resolution (004) happens here, for every shard configured, before
Runtime begins — strictly before any worker thread is created. `ResourcePlan`
references the resolved Node/Shard resource slots by dense index; no lookup
occurs at activation time for these two lifetimes, and a Node-scoped resource
shared by many shards is constructed exactly once, by the single thread
executing `build()`, with zero atomics, locks, or `std::call_once` — the
multi-shard first-touch race is dissolved by ordering (resolution completes
before any worker thread exists to race), not resolved by synchronization.
...
- Proven: collision → `unexpected`, table pointer-identical; add-vs-drain TSan+ASan clean,
  0 bad records/cells.
- **Resource-type closed-world constraint (ADR-021, proven).** A guarded
  `add_actor_type<T>()` whose `ResourcePlan` references a Node- or Shard-scoped
  resource type never registered at the original `build()` **must fail
  incremental Validation** — `std::unexpected`, publish nothing, resource
  tables pointer-identical — rather than attempt live resolution against
  running workers. The zero-atomics guarantee for Node/Shard resolution is a
  closed-world guarantee over the resource-type set known at first `build()`.
...
- *(Node/Shard resource resolution ordering vs. actor activation on a cold
  shard: resolved — resolved eagerly for every shard inside this cold
  metadata-compilation phase, strictly before any worker thread exists; see
  "Metadata compilation" above, ADR-021.)*
