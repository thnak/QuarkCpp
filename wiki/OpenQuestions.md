...
All twenty-six subsystem specs are now drafted (see [Contributing § Reading order](Contributing#reading-order)). What
...
- **Outbound streaming replies — an `ask` that returns a stream** (006 + 024 + 017 + 018 +
  ...
  in [ADR-018](ADR-018-outbound-streaming-replies) by a 3-design debate-prove
...
- **Actor execution vehicle — passive+stackless vs green threads/fibers** (001/002/015/024/023)
  — resolved in [ADR-015](ADR-015-actor-execution-vehicle-passive-stackless-vs-fibers)
...
- **Reentrancy / quiescence** (was the top cross-cutting hazard, touching
  ...
  [015-Reentrancy-and-Quiescence](015-Reentrancy-and-Quiescence): reentrant
...
- **Serialization, one story for three consumers** (wire 010, snapshot + event-log
  012) — resolved in [016-Serialization](016-Serialization): one
  ...
  **Accepted (x86-64)** — the encode-budget + negotiation/evolution gate is met
  ([ADR-016](ADR-016-serialization-wire-fast-path-encode-gate)): tagless
...
- **Effectively-once effects** (wire 010 + fenced persistence 012 + identity 016)
  — resolved in [017-Delivery-Guarantees](017-Delivery-Guarantees): three
...
- **Cross-node time** (004 context + 006 ask + 010 transport + 011 deadlines) —
  resolved in [018-Clocks-and-Deadlines](018-Clocks-and-Deadlines):
...
- **Platform Abstraction Layer** (the OS seam under 002/010/012/018) — resolved in
  [019-Platform-Abstraction-Layer](019-Platform-Abstraction-Layer): one
...
- **Security** (distribution 010 + context 004/006 + secrets 013 + persistence 012
  + PAL 019 + observability 009) — resolved in [020-Security](020-Security):
...
- **Inbound streaming & large-message cost** (messaging 006 + resources 004 +
  ...
  [024-Streaming-and-Inbound-Streams](024-Streaming-and-Inbound-Streams) and
  proven by [ADR-005](ADR-005-inbound-stream-ingestion-hot-path): a
  ...
  **Accepted (x86-64)** — the promotion gate is met:
  [ADR-014](ADR-014-streaming-async-suspend-real-scheduler-gate) wired the
...
- **Placement optimization & stateless workers** (placement 010 + policies 005 +
  ...
  **no identity / no per-key FIFO / non-durable**, the one explicit relaxation of the
  ...
  PROVEN** ([ADR-011](ADR-011-cluster-relay-and-placement-gate-verification) Gate C
  ...
  independent multinomial ([ADR-013](ADR-013-weighted-hrw-distribution-regate-2):
...
- **Large-scale cluster topology** (distribution 010 + cluster 021 + placement 025 +
  ...
  [026-Large-Scale-Cluster-Topology](026-Large-Scale-Cluster-Topology) and proven
  by [ADR-006](ADR-006-large-scale-cluster-topology): scaling to 10³–10⁴
  ...
  gate is **proven** ([ADR-011](ADR-011-cluster-relay-and-placement-gate-verification):
  ...
  by 025's re-gate ([ADR-013](ADR-013-weighted-hrw-distribution-regate-2) CORRECT —
...
- **Developer-facing authoring & handler-dispatch API** (developer model 005 +
  ...
  as **one coherent API** in [ADR-007](ADR-007-actor-authoring-and-handler-dispatch-api)
...
- **Configuration + activation-lifecycle policy** (config 013 + metadata/startup 008 +
  ...
  surface** in [ADR-008](ADR-008-engine-actor-configuration-and-activation-lifecycle-policy)
...
- **Failure, supervision & recovery policy** (failure 007 + resources 004 + quiescence 015
  ...
  [ADR-009](ADR-009-failure-supervision-and-recovery-policy-model) (D1
...
- **Priority & fairness scheduling** (scheduler 002 + developer model 005 + timers 011 +
  perf 023) — resolved in [ADR-010](ADR-010-priority-and-fairness-scheduling-policy)
...
3. **Stream async-suspend resume against the real scheduler** (024/002/015,
   ADR-005) — **RESOLVED** ([ADR-014](ADR-014-streaming-async-suspend-real-scheduler-gate)).
...
- *(Empty — every subsystem the RFC set out to cover is now drafted, including
  ...
  [ADR-018](ADR-018-outbound-streaming-replies)** (above). What remains is all
...
- C++23, hybrid handler model, CRTP policy types — see [Home](Home).
