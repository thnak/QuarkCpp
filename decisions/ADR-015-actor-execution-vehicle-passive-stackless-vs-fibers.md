...
7. **[FOUND AND FIXED 2026-08-04] `Activation::complete_parked()` had a lost-wakeup/UB race against `drain_step`'s own suspend tail** — the exact bug class this ADR's own §110/Fibra-withdrawal analysis already named once (the suspend branch racing the release-store that publishes the parked frame), but it had not actually been closed on the Sequential `complete_parked()` path this ADR shipped. A carrier calling `complete_parked()` before `drain_step` finished writing `parked_frame_`/`parked_desc_` and calling `exec_.park()` raced those plain fields (UB) and could permanently strand the activation in `Parked` (the `readmit_from_parked()` CAS fails silently, no retry). Dormant in shipped code only because no `blocking<>`/`fiber<>` adapter — the intended caller — was ever implemented; surfaced while scoping 006's `ask`/`ask_stream` co_await re-admit wiring, which would have been the first real caller. Red-teamed (429/429 reproductions under a 5 µs injected delay against the real `ExecStateCell`) and fixed by gating `complete_parked()`'s reads behind an acquire-confirmed observation of `ExecState::Parked` (the standard release/acquire message-passing idiom over the existing `park()` release-store — no new primitive). Proven with a permanent test + its disable-the-fix control (`tests/activation_park_race_test.cpp`, `activation_park_race_control`): fixed build 0/600 stuck across widened-window iterations, control 600/600 stuck, full 198-test suite green under both the default config and ASan+UBSan. TSan unavailable on the proving host (confirmed, not assumed) — this proof's cross-thread evidence is the injected-delay stress technique plus ASan/UBSan, not TSan; a TSan re-run on Linux CI is a recommended follow-up before any `blocking<>`/`fiber<>` adapter is built on top of this seam.
